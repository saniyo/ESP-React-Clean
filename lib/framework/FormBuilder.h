...
inline const char* toString(FieldType ft) {
  switch (ft) {
    ...
    case FieldType::BUTTON:   return "button"; // додано
...
class FormBuilder {
 public:
  ...
  // ---------- BUTTON ----------
  template <typename... Args>
  static JsonObject addButtonField(JsonArray& fields,
                                   const char* key,
                                   AF accessFlag,
                                   bool value,
                                   Args... btnArgs) {
    // опційно лише placeholder
    const char* pl = nullptr;
    parseButtonArgs(pl, btnArgs...);

    JsonObject field = fields.createNestedObject();
    field[key] = value ? "1" : "0";  // консистентно зі switch/checkbox
