...
//   slider:   { options: ['r', 'rw', 'mn', 'mx', 'f', 'pl', 'st'] as const },
...
//   trend:    {
//     // Видаляємо 'data'
//     options: ['to', 'xAxis', 'lines', 'legend', 'tooltip', 'maxPoints', 'mode'] as const
//   }
// };
...
//   mn: (opt) => ({ key: 'mn', value: parseFloat(opt.slice(2)) }),
//   mx: (opt) => ({ key: 'mx', value: parseFloat(opt.slice(2)) }),
//   f:  (opt) => ({ key: 'f',  value: parseFloat(opt.slice(1)) }),
//   pl: (opt) => ({ key: 'pl', value: opt.slice(2) }),
//   st: (opt) => ({ key: 'st', value: parseFloat(opt.slice('st='.length)) }),

//   to:      (opt) => ({ key: 'to',      value: opt.slice('to='.length) }),
//   legend:  ()   => ({ key: 'legend',  value: true }),
//   tooltip: ()   => ({ key: 'tooltip', value: true }),

//   // lines=key1:color=...,key2:color=...
//   lines: (opt) => {
//     const val = opt.slice('lines='.length);
//     return { key: 'lines', value: val };
//   },

//   // options=Option1,Option2
//   options: (opt) => {
//     const val = opt.slice('options='.length);
//     const items = val.split(',').map(s => s.trim());
//     return { key: 'options', value: items };
//   },
//   maxPoints: (opt) => {
//     const val = opt.slice('maxPoints='.length);
//     return { key: 'maxPoints', value: parseInt(val, 10) };
//   },
//   mode: (opt) => {
//     const val = opt.slice('mode='.length).trim();
...
//   )
// );
// allKnownKeys.sort((a, b) => b.length - a.length);
...
//   let splitted = optionsString.split(';').map(s => s.trim());
...
//     if (prefix && optionParsers[prefix]) {
//       const parsed = optionParsers[prefix](option);
//       optionMap[parsed.key] = parsed;
//     } else if (prefix && (config.options as readonly string[]).includes(prefix as FieldOptionKey)) {
//       // Наприклад "legend" без "=true"
//       optionMap[prefix as FieldOptionKey] = { key: prefix as FieldOptionKey, value: true };
//     } else if ((config.options as readonly string[]).includes(option as FieldOptionKey)) {
//       // Типу "rw", "r" і тд
//       optionMap[option as FieldOptionKey] = { key: option as FieldOptionKey, value: true };
//     }
//   });
